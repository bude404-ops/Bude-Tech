 ...
 * FIXED: Finite builds with max cycles + completion exit
 */
...
const CONFIG = {
  ...
  cycleDelay: 1200,
  maxCycles: 15,
  maxFeatures: 3  // Build top 3 features then stop
};
...
function planner() {
  // If we've built enough features, we're done
...
function devops() {
  ...
  // FIX: Go back to init to pick next feature, OR done if enough built
  if (STATE.completed.length >= CONFIG.maxFeatures) {
    STATE.phase = "done";
  } else {
    STATE.phase = "init";
...
async function loop() {
  ...
  if (STATE.cycle >= CONFIG.maxCycles && STATE.phase !== "done") {
    log("⚠️ MAX CYCLES REACHED");
    STATE.phase = "done";
  }

  log("✅ BUILD COMPLETE");
  log("Built features: " + STATE.completed.join(", "));
  saveState();
}
