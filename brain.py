...
# ==========================================================
# BudE Tech Brain V2 (SELF-UPGRADE CORE)
# ==========================================================
...
UPGRADE_TRIGGER_KEYWORDS = ["upgrade", "improve", "optimize", "better", "fix"]
...
# ----------------------------------------------------------
# SELF-UPGRADE ENGINE (CONTROLLED)
# ----------------------------------------------------------

def should_upgrade(task_text):
    t = task_text.lower()
    return any(k in t for k in UPGRADE_TRIGGER_KEYWORDS)


def generate_upgrade(task_text):
    """
    AI proposes improvement (NOT applied directly)
    """

    change = {
        "file": "brain.py",
        "improvement": "Refactor worker routing logic",
    ...
    if not safety_rules.is_safe(str(change)):
        log("UPGRADE BLOCKED BY SAFETY RULES")
        return

    upgrade_engine.propose_upgrade(
...
def auto_mode():
    ...
    while True:
        ...
        for task in pending:
            ...
            # SELF-UPGRADE HOOK
            if should_upgrade(task["task"]):
                generate_upgrade(task["task"])
