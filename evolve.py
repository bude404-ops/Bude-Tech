...
def business_phase_prompt(memory, files):
    ...
    # FIXED: Match actual file names that were built
    opportunities = [
...
- DO NOT rebuild files that already exist — check repo state first
"""
...
"""
    ...
    # FIXED: Add already-built files to prompt so AI doesn't rebuild them
    already_built = memory.get("business_modules", []) + memory.get("modules_built", [])
    already_built_section = ""
    if already_built:
        already_built_section = f"""
ALREADY BUILT (DO NOT rebuild these):
{json.dumps(already_built[-10:], indent=2)}
"""
...
- DO NOT create files that already exist in the repo
- If a file exists, you may modify it but mention it in upgrades_made
...
def try_models(prompt):
    for model in GROQ_MODELS:
        try:
            ...
            # FIXED: Rate limit backoff
            if "429" in err_msg or "Too Many Requests" in err_msg:
                log_event("Rate limited, waiting 30s before next model...", "WARN")
                time.sleep(30)
            continue
...
def apply_changes(result, memory):
    ...
    for action in result.get("actions", []):
        ...
        # FIXED: Skip if file already exists and content is similar
