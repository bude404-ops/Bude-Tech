...
# Files that CAN be updated (evolve.py, brain.md, system/*, dashboard files)
ALLOWED_UPDATES = [
    "evolve.py",
    "brain.md",
    "dashboard.js",
...
def business_phase_prompt(memory, files):
    """Revenue-focused — only build missing money-making files."""
    # Define opportunities with exact file paths
    all_opportunities = {
    ...
    # Check which files actually exist in repo
    existing_files = set(files)
    built_biz = set(memory.get("business_modules", []))
    
    # Find truly missing files (not in repo AND not in business_modules)
    missing = []
    ...
    # If nothing missing, allow improvements to allowed files
    if not missing:
...
"""
    ...
    # List existing files to prevent rebuilds
...
def apply_changes(result, memory):
    ...
    for action in result.get("actions", []):
        ...
        # HARD BLOCK: Check if file exists and is NOT in allowlist
        if os.path.exists(path):
            ...
            if not is_allowed and not is_system:
                ...
                # Allowed file — check if content actually changed
                try:
